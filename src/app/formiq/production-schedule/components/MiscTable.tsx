...
export default function MiscTable({ miscItems, userAccess, companyId }: MiscTableProps) {
  ...
  const handleAddPO = (e: React.MouseEvent, miscId: string) => {
    e.stopPropagation()
    setSelectedItemForPO(miscId)
            ...
            {miscItems.map((miscItem) => (
              <React.Fragment key={miscItem.id}>
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {formatDate(miscItem.original_scheduled_ship_date)}
                  </td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${getShipDateColorClass(miscItem.original_scheduled_ship_date, miscItem.current_scheduled_ship_date)}`}>
