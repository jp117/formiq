...
import { MiscItem, MiscFormData } from '../../types'
import CommonFormFields from '../shared/CommonFormFields'

interface EditMiscModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function EditMiscModal({ 
  isOpen, 
  onClose, 
  miscItem,
  userAccess,
  ...
  // Populate form when misc item changes
  useEffect(() => {
    if (miscItem) {
      setFormData({
        quantity: miscItem.quantity,
        description: miscItem.description,
        sales_order_number: miscItem.sales_order_number,
  ...
  if (!isOpen || !miscItem) return null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating misc item:', error)
      alert('Error updating misc item. Please try again.')
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Edit Misc Item: {miscItem.description}</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {isSubmitting ? 'Updating...' : 'Update Misc Item'}
            </button>
          </div>
        </form>
      </div>
