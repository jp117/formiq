...
import { SwitchboardFormData } from '../../types'
import CommonFormFields from '../shared/CommonFormFields'

interface CreateSwitchboardModalProps {
  isOpen: boolean
  onClose: () => void
  companyId: string
  onItemCreated: () => void
...
export default function CreateSwitchboardModal({ 
  isOpen, 
  onClose, 
  companyId, 
  onItemCreated 
}: CreateSwitchboardModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error creating switchboard. Please try again.')
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Create New Switchboard</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {isSubmitting ? 'Creating...' : 'Create Switchboard'}
            </button>
          </div>
        </form>
      </div>
